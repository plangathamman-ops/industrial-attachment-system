...
Complete guide to deploy your application to AWS EKS with ArgoCD GitOps.
...
4. [ECR Setup](#ecr-setup)
5. [Build & Push Docker Images](#build-push-docker-images)
6. [ArgoCD Installation](#argocd-installation)
7. [Deploy Application](#deploy-application)
8. [Configure Domain & SSL](#configure-domain-ssl)
...
- Use 1 node initially: Save $30/month
...
### Step 2: Create IAM User for Deployment (Optional but Recommended)

```bash
# Create IAM user
aws iam create-user --user-name eks-deployment-user

# Attach required policies
aws iam attach-user-policy \
  --user-name eks-deployment-user \
...
# Create access key
aws iam create-access-key --user-name eks-deployment-user
